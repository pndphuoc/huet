...
class SearchTouristAttractionPage extends StatefulWidget {
  ...
  const SearchTouristAttractionPage({Key? key, required this.callback}) : super(key: key);

  @override
  State<SearchTouristAttractionPage> createState() => _SearchTouristAttractionPageState();
}

class _SearchTouristAttractionPageState extends State<SearchTouristAttractionPage> {
  @override
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      appBar: AppBar(
        backgroundColor: colors.backgroundColor,
        elevation: 0,
        title: Text("Choose a tourist attraction", style: GoogleFonts.readexPro(color: Colors.black),),
    ...
    return Container(
      ...
      child: GestureDetector(
        ...
        child: Column(
          ...
          children: [
            Text(att.title, style: GoogleFonts.readexPro(color: Colors.black, fontSize: 20),),
            Text(att.address, style: GoogleFonts.readexPro(fontSize: 15, color: Colors.grey,), maxLines: 1, overflow: TextOverflow.ellipsis,),
          ],
        ),
      ),
    );
